...
class Parser:
    ...
    def __postfix(tokens: List[LexicalToken]):
        ...
        stack = []
        for token in tokens:
            if token in [TT_SYMBOL, TT_INT, TT_REAL, TT_FUNC, TT_LCURLY, TT_RCURLY]:
                postfix_expr.append(token)
            elif token == TT_LPAREN:
                stack.append(token)
            elif token == TT_RPAREN:
            ...
            elif token in [TT_PLUS, TT_MINUS, TT_MULTIPLY, TT_DIVIDE, TT_EXPONENT]:
